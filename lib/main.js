module.exports = function() {
  ...
  Question.prototype.getRawAnswer = function() {
  ...
  Survey.prototype.initDecisionTree = function(data, label, features) {
    if (utility.basic.isArray(data) === false) { throw new Error('Data type error.'); }
    if (utility.basic.isNull(data === true)) { throw new Error('Data type error.'); }
    this._dt = {
      'data': data,
      'label': label,
      'features': features,
      'tree': new DecisionTree(data, label, features)
    };

    // [TODO] give out evaluation
    return this;
  };
  ...
  Survey.prototype.evaluate = function() {
    ...
    for (var key in this._questions) {
      ...
      if (_.indexOf(this._dt.features, key) !== -1) {
        data[key] = question.getRawAnswer();
      }
    }
    return {
      'label': this._dt.tree.predict(data),
      'point': point
    };
  };
